...
table(df$drink) %>%
  as.data.frame() %>%
  mutate("Drinking Status" = ifelse(Var1 == 0, "Involved drinking driver",
...
-   Tabulation for continuous variables
...
-   Independent sample T-Tests for the continuous variables
...
r_matrix %>%
  kable(caption = "Table 3. Correlation matrix of predictors") %>%
...
p_matrix %>%
  kable(caption = "Table 4. Correlation matrix (p-values) of predictors") %>%
  kable_classic(html_font="Cambria")
```
